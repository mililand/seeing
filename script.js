...
// === Theme switcher ===
function themeHrefFor(name){
  if (name === 'emerald') return 'theme-emerald.css?v=1';
  if (name === 'noir')    return 'theme-noir.css?v=1';
  if (name === 'warm')    return 'theme-warm.css?v=1';
  return '';
}
function applyTheme(name){
  try{
    var link = document.getElementById('themeStyles');
    if (!link) return;
    var href = themeHrefFor(name);
    link.setAttribute('href', href);
    localStorage.setItem('seeing_theme', name || '');
    // Set pressed state on buttons
    var sw = document.getElementById('themeSwitcher');
    if (sw){
      sw.querySelectorAll('button[data-theme]').forEach(function(b){
        var on = (b.getAttribute('data-theme') === name);
        b.classList.toggle('sel', on);
        b.setAttribute('aria-pressed', on ? 'true' : 'false');
      });
    }
  }catch(e){ console && console.warn && console.warn('theme switch', e); }
}
function initThemeSwitcher(){
  ...
  sw.addEventListener('click', function(e){
    ...
    if (t && t.matches('button[data-theme]')){
      var name = t.getAttribute('data-theme');
      applyTheme(name);
    }
  });
  // Load from storage on boot
  var saved = localStorage.getItem('seeing_theme') || 'warm';
  applyTheme(saved);
}
